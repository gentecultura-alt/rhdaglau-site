...
const FAQ = () => {
  return (
    <section id="faq" className="py-20 bg-gradient-to-br from-sky-50 to-orange-50">
      <div className="container max-w-3xl">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-sky-100 p-6 md:p-8">
          <Accordion type="single" collapsible className="w-full">
            {faqItems.map((item, index) => (
              <AccordionItem key={index} value={`item-${index}`} className="border-sky-100">
                <AccordionTrigger className="text-left font-medium text-gray-700 hover:text-sky-600 hover:no-underline py-4">
                  {item.question}
                </AccordionTrigger>
